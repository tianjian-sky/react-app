...
import * as utils from '../../lib/cuon-matrix'
...
export default class extends React.Component {
    ...
    initDraw() {
        ...
        var tick = () => {
            angel+=1
            this.draw(gl, u_xformMatrix, u_mat, u_FragColor, n, angel)
            requestAnimationFrame(tick)
        }
        tick()
    }

    draw(gl, u_xformMatrix, u_mat, u_FragColor, n, angel) {
        u_mat.setRotate(angel, 1, 1, 0)
        gl.uniformMatrix4fv(u_xformMatrix, false, u_mat.elements)
        gl.uniform4f(u_FragColor, Math.random(), Math.random(), Math.random(), 1)
        gl.clearColor(0,0.222,.333,1)
        gl.clear(gl.COLOR_BUFFER_BIT)
        gl.drawArrays(gl.TRIANGLE_STRIP, 0, n)
